...
type mocktracerV2Adapter struct {
	Tracer v2mock.Tracer
}

// FinishedSpans implements Tracer.
func (ma *mocktracerV2Adapter) FinishedSpans() []Span {
	spans := ma.Tracer.FinishedSpans()
	converted := make([]Span, len(spans))
	for i, s := range spans {
		converted[i] = &spanV2Adapter{Span: s}
	}
	return converted
}

// OpenSpans implements Tracer.
func (ma *mocktracerV2Adapter) OpenSpans() []Span {
	spans := ma.Tracer.OpenSpans()
	converted := make([]Span, len(spans))
	for i, s := range spans {
		converted[i] = &spanV2Adapter{Span: s}
	}
...
func (ma *mocktracerV2Adapter) Reset() {
	ma.Tracer.Reset()
}

// SentDSMBacklogs implements Tracer.
func (ma *mocktracerV2Adapter) SentDSMBacklogs() []datastreams.Backlog {
	backlog := ma.Tracer.SentDSMBacklogs()
	converted := make([]datastreams.Backlog, len(backlog))
	for i, b := range backlog {
		converted[i] = datastreams.Backlog{
			Tags:  b.Tags,
			Value: b.Value,
		}
	}
...
func (ma *mocktracerV2Adapter) Stop() {
	ma.Tracer.Stop()
}
...
func Start() Tracer {
	v2mock.Start()
	return &mocktracerV2Adapter{
		Tracer: tracer.GetGlobalTracer().(v2mock.Tracer),
	}
}
